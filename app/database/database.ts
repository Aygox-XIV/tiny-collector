...
function createDB(files: FileCollection): Database {
    ...
    let licenseCounts = {} as Record<Category, number>;
    Object.values(items).forEach((i) => {
        itemCounts[i.category] = (itemCounts[i.category] || 0) + 1;
        if (i.license_amount) {
            licenseCounts[i.category] = (licenseCounts[i.category] || 0) + 1;
        }
    });
    console.log(`distribution: ${JSON.stringify(itemCounts)}`);
    console.log(`license distribution: ${JSON.stringify(licenseCounts)}`);
